...
const handlers = [
  ...
  http.get(
    ...
    ({ params, request }) => {
      ...
      return HttpResponse.json({
        verses: [{ ...mockVerse, chapter_id: parseInt(params.chapter_id) }],
        pagination: {
          page: parseInt(page),
  ...
  http.get('https://api.qurancdn.com/api/qdc/verses/by_juz/:juz', ({ params, request }) => {
    ...
    const perPage = url.searchParams.get('per_page') || '10';
    return HttpResponse.json({
      verses: [{ ...mockVerse, juz_number: parseInt(params.juz) }],
      pagination: {
        page: parseInt(page),
        per_page: parseInt(perPage),
        total_pages: 1,
  ...
  http.get('https://api.qurancdn.com/api/qdc/verses/by_page/:page', ({ params, request }) => {
    ...
    const perPage = url.searchParams.get('per_page') || '10';
    return HttpResponse.json({
      verses: [{ ...mockVerse, page_number: parseInt(params.page) }],
      pagination: {
    ...
    ({ params, request }) => {
      ...
      return HttpResponse.json({
        translations: [
          {
            ...mockTranslation,
            resource_id: parseInt(params.translation_id),
  ...
  http.get('https://api.qurancdn.com/api/qdc/quran/tafsirs/:tafsir_id', ({ params, request }) => {
    ...
    return HttpResponse.json({
      tafsirs: [
        {
          ...mockTafsir,
          resource_id: parseInt(params.tafsir_id),
          verse_key: `${chapterNumber}:${verseNumber}`,
        },
      ],
    });
  ...
  http.get('https://api.quran.com/api/v4/chapters/:id', ({ params }) => {
    return HttpResponse.json({
      chapter: { ...mockChapter, id: parseInt(params.id) },
    });
  }),

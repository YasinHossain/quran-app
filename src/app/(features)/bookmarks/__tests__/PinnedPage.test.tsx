...
jest.mock('@/presentation/providers/BookmarkContext', () => ({
  useBookmarks: () => ({
    pinnedVerses,
    removeBookmark,
    chapters: [
...
jest.mock('@/presentation/(features)/layout/context/HeaderVisibilityContext', () => ({
  useHeaderVisibility: () => ({ isHidden: false }),
}));

jest.mock('../components/BookmarkCard', () => ({
  BookmarkCard: ({ bookmark }: any) => {
    const { removeBookmark } = require('@/presentation/providers/BookmarkContext').useBookmarks();
    return (
      <div>

...
async function generateIcons() {
  ...
  console.log(
    `Content ratio: ${CONTENT_RATIO * 100}% (padding: ${(((1 - CONTENT_RATIO) / 2) * 100).toFixed(1)}% each side)`
  );
  ...
  for (const size of sizes) {
    ...
    console.log(
      `✓ Generated icon-${size}x${size}.png (${contentSize}px content + ${padding}px padding)`
    );
  }

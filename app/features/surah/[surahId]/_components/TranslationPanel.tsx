      ...
      <div className={`fixed top-0 right-0 w-80 h-full bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 flex flex-col transition-transform duration-300 ease-in-out z-50 shadow-lg ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-3 border-b border-gray-200/80">
          <div className="relative">
            ...
            <input type="text" placeholder={t('search')} value={searchTerm} onChange={e => onSearchTermChange(e.target.value)} className="w-full pl-10 pr-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 placeholder:text-slate-500 dark:placeholder:text-slate-400" />
          </div>
        </div>

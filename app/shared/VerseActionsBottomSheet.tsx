...
const VerseActionsBottomSheet: React.FC<VerseActionsBottomSheetProps> = ({
  isOpen,
  onClose,
  verseKey,
  isPlaying,
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              ...
              <button
                onClick={onClose}
                className="p-2 rounded-full hover:bg-interactive transition-colors touch-manipulation"
                aria-label="Close"
              >
                ...
                {actions.map((action, index) =>
                  action.href ? (
                    <Link
                      ...
                      className={`flex items-center gap-4 p-4 rounded-2xl hover:bg-interactive transition-all duration-200 touch-manipulation ${
                        action.active ? 'text-accent' : 'text-foreground'
                      }`}
                    >
                      <div
                        className={`flex-shrink-0 ${action.active ? 'text-accent' : 'text-muted'}`}
                      >
                        {action.icon}
                      </div>
                    ...
                    <motion.button
                      key={index}
                      onClick={action.onClick}
                      className={`flex items-center gap-4 p-4 rounded-2xl hover:bg-interactive transition-all duration-200 touch-manipulation w-full text-left ${
                        action.active ? 'text-accent' : 'text-foreground'
                      }`}
                      whileTap={{ scale: 0.98 }}
                    >
...
export default VerseActionsBottomSheet;
